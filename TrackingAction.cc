...
{ 
  fXstartAbs = fXendAbs = fPrimaryCharge = fDirX = x0_outOfWorld = 0.0;
}
...
void TrackingAction::PreUserTrackingAction(const G4Track* aTrack )
{
  ...
  if (aTrack->GetTrackID() == 1) {
    ...
    x0_outOfWorld = aTrack->GetPosition().x();
//  G4cout << " @ fDirX : " << fDirX << G4endl;  
//  G4cout << " @ xStartAbs : " << fXendAbs << G4endl;  
//  G4cout << " @ xEndAbs : " << fXendAbs << G4endl;  
  }
}
...
/*
  ...
  G4cout << " @ Track Def   : " << aTrack->GetDefinition() <<  G4endl;
  G4cout << " @ Charge PDG  : " << charge <<  G4endl;
...
//  G4string lastVolume = aTrack->GetVolume()->GetName();
 
  G4bool ejected = ( (aTrack->GetVolume()->GetName()=="World") && ( aTrack->GetMomentumDirection().x() < 0.0  )  
		  //&& (position.x() < fXstartAbs) 
		  && energy > 0.0 );
 // G4cout << " @isSecond    : " << ejected << G4endl;

//  G4cout << " @ Process   : " << aTrack->GetPostStepPoint() << G4endl;
